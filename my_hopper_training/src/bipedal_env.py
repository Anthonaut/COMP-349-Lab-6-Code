...
from bipedal_state import bipedalState
from controllers_connection import ControllersConnection

#register the training environment in the gym as an available one
reg = register(
    id='bipedal-v0',
    entry_point='bipedal_env:bipedalEnv',
    timestep_limits=50 # This is causing runtime issues
    )
...
class bipedalEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.controllers_object = ControllersConnection(namespace="bipedal")

        self.bipedal_state_object = bipedalState(   max_height=self.max_height,
        ...
        self.bipedal_state_object.set_desired_world_point(self.desired_pose.position.x,
                                                          self.desired_pose.position.y,
                                                          self.desired_pose.position.z)

        self.bipedal_joint_pubisher_object = JointPub()
    ...
    def _reset(self):
        ...
        rospy.logdebug("reset_bipedal_joint_controllers...")
        self.controllers_object.reset_bipedal_joint_controllers()

        # 3rd: resets the robot to initial conditions
        rospy.logdebug("set_init_pose...")
        self.bipedal_joint_pubisher_object.set_init_pose()

        # 5th: Check all subscribers work.
        # Get the state of the Robot defined by its RPY orientation, distance from
        # desired point, contact force and JointState of the three joints
        rospy.logdebug("check_all_systems_ready...")
        self.bipedal_state_object.check_all_systems_ready()
        rospy.logdebug("get_observations...")
        observation = self.bipedal_state_object.get_observations()
    ...
    def _step(self, action):
        ...
        next_action_position = self.bipedal_state_object.get_action_to_position(action)

        # We move it to that pos
        self.gazebo.unpauseSim()
        self.bipedal_joint_pubisher_object.move_joints(next_action_position)
        ...
        reward,done = self.bipedal_state_object.process_data()
    ...
    def get_state(self, observation):
        ...
        :return: state
        """
        return self.bipedal_state_object.get_state_as_string(observation)
