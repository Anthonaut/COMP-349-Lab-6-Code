...
"""
 wrenches:
  ...
  -
    info: "Debug:  i:(2/4)     my geom:bipedal::lowerleg::lowerleg_contactsensor_link_collision_1\
  \   other geom:ground_plane::link::collision         time:50.405000000\n"
    collision1_name: "bipedal::lowerleg::lowerleg_contactsensor_link_collision_1"
...
class bipedalState(object):
    ...
    def __init__(self, max_height, min_height, abs_max_roll, abs_max_pitch, joint_increment_value = 0.05, done_reward = -1000.0, alive_reward=10.0, desired_force=7.08, desired_yaw=0.0, weight_r1=1.0, weight_r2=1.0, weight_r3=1.0, weight_r4=1.0, weight_r5=1.0, discrete_division=10):
        rospy.logdebug("Starting bipedalState Class object...")
        ...
        rospy.Subscriber("/bipedal/imu/data", Imu, self.imu_callback)
        ...
        rospy.Subscriber("/bipedal/joint_states", JointState, self.joints_state_callback)
    ...
    def check_all_systems_ready(self):
        ...
        while imu_data is None and not rospy.is_shutdown():
            try:
                imu_data = rospy.wait_for_message("/bipedal/imu/data", Imu, timeout=0.1)
        ...
        while joint_states_msg is None and not rospy.is_shutdown():
            try:
                joint_states_msg = rospy.wait_for_message("/bipedal/joint_states", JointState, timeout=0.1)
    ...
    def set_desired_world_point(self, x, y, z):
        """
        Point where you want the bipedal to be
        :return:
        """
    ...
    def get_action_to_position(self, action):
        ...
        if action == 0: #Increment L_haa_joint
            ...
            action_position[1] = joint_states_position[1]
            action_position[2] = joint_states_position[2] - self._joint_increment_value
        elif action == 6: #Increment R_haa_joint
            action_position[0] = joint_states_position[0] + self._joint_increment_value
            action_position[1] = joint_states_position[1]
            action_position[2] = joint_states_position[2]
    ...
    def process_data(self):
        ...
        :return: reward, done
        """
        bipedal_height_ok = self.bipedal_height_ok()
        bipedal_orientation_ok = self.bipedal_orientation_ok()

        done = not(bipedal_height_ok and bipedal_orientation_ok)
        if done:
...
if __name__ == "__main__":
    rospy.init_node('bipedal_state_node', anonymous=True)
    bipedal_state = bipedalState(max_height=3.0,
                                 min_height=0.6,
                                 abs_max_roll=0.7,
                                 abs_max_pitch=0.7)
    bipedal_state.testing_loop()
